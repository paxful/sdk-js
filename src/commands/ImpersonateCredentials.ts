    ...
    return fetch(createOAuthRequestTokenUrl(config, code))
        .then(async response => {
            if (!response.ok) {
                ...
                throw Error(`Invalid response received (expected 200, received ${response.status}) when trying  to retrieve impersonated credentials: ${errText}.`);
            }
        ...
        .then((tokenResponse: AccountServiceTokenResponse) => {
            if (!tokenResponse.access_token || !tokenResponse.expires_in) {
                throw Error(`Invalid response received when trying to retrieve impersonated credentials - server didn't return a properly formatted token.`);
            }
            return ({
